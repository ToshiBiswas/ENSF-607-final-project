...
export function EventPage({ eventId }: { eventId: number }) {
  ...
  if (loading) return <div className="event-page">Loading event…</div>;
  if (error || !event) return <div className="event-page error">{error ?? 'Event missing'}</div>;
  ...
  return (
    <section className="event-page">
      <header className="event-hero">
        <p className="event-hero__eyebrow">{event.categories.map((c) => c.value).join(' • ')}</p>
        <h1>{event.title}</h1>
        <p className="event-hero__meta">
          {event.location} • {formatDate(event.startTime)} – {formatDate(event.endTime)}
        </p>
        <p className="event-hero__host">Hosted by {event.organizer?.name ?? 'Unknown'}</p>
      </header>
