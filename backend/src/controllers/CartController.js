...
class CartController {
  ...
  static add = asyncHandler(async (req, res) => {
    ...
    const cart = await TicketingService.addToCart({ userId: req.user.userId }, Number(ticket_info_id), Number(quantity));
    res.status(201).json({ cart });
  });

  /** GET /api/cart */
  static view = asyncHandler(async (req, res) => {
    const cart = CartService.getCart({ userId: req.user.userId });
    res.json({ cart, total_cents: cart.totalCents() });
  });

  /** DELETE /api/cart  (clear) */
  static clear = asyncHandler(async (req, res) => {
    const cart = CartService.clear({ userId: req.user.userId });
    res.json({ cart });
  });
